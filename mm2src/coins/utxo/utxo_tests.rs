...
#[ignore]
#[test]
fn block_header_utxo_loop_test() {
    lazy_static! {
        static ref EXPECTED_HAEDERS: Mutex<HashMap<u64, BlockHeader>> = Mutex::new(HashMap::new());
    };
    use mm2_test_helpers::for_tests::KMD_ELECTRUM_ADDRS;

    use crate::utxo::utxo_builder::{BlockHeaderUtxoArcOps, UtxoCoinBuilder};

    ElectrumClientImpl::new.mock_safe(|_, event, _, abort| {
        let block_headers_storage = BlockHeaderStorage {
            inner: Box::new(utxo_sql_block_header_storage::BlockHeaderInMemoryStorage {
                ticker: "KMD".to_string(),
                conn: Arc::new(Mutex::new(Connection::open_in_memory().unwrap())),
            }),
        };

        let client = ElectrumClientImpl::new("KMD".to_owned(), event, block_headers_storage, abort);

        MockResult::Return(client)
    });
    BlockHeaderStorage::add_block_headers_to_storage.mock_safe(|f, header| {
        println!("HHII {:?}", header);
        // EXPECTED_HAEDERS
        //     .lock()
        //     .unwrap()
        //     .remove(..)
        //     .expect("Tried to upload an unexpected header {hash}");

        MockResult::Return(Box::pin(futures::future::ok(())))
    });
    ElectrumClient::get_block_count.mock_safe(|_| MockResult::Return(Box::new(futures01::future::ok(10280))));
    BlockHeaderStorage::get_last_block_height.mock_safe(|_| MockResult::Return(Box::pin(futures::future::ok(10250))));

    let ctx = MmCtxBuilder::default().into_mm_arc();
    let servers: Vec<_> = KMD_ELECTRUM_ADDRS
        .iter()
        .map(|server| json!({ "url": server, "protocol": "SSL", "disable_cert_verification":true }))
        .collect();
    let req = json!({
        "method": "electrum",
        "servers": servers,
    ...
    block_on(builder.electrum_client(abortable_system, args, servers)).unwrap();
    let field = block_on(builder.build_utxo_fields()).unwrap();
    let arc: UtxoArc = field.into();
    builder.spawn_block_header_utxo_loop(
        &arc,
        UtxoStandardCoin::from,
        block_on(builder.build_utxo_fields())
            .unwrap()
    ...
    loop {
        let all_uploaded = EXPECTED_HAEDERS.lock().unwrap().is_empty();
        if all_uploaded {
            break;
        }
    }

    ElectrumClient::get_block_count.mock_safe(|_| MockResult::Return(Box::new(futures01::future::ok(10300))));
    BlockHeaderStorage::get_last_block_height.mock_safe(|_| MockResult::Return(Box::pin(futures::future::ok(10280))));
    builder.spawn_block_header_utxo_loop(
        &arc,
        UtxoStandardCoin::from,
        block_on(builder.build_utxo_fields())
